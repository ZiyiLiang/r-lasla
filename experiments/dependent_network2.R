...
nrep=100
q<-0.05
adjustment.vec <- rev(c(1,1.5,2,3,5,10))       # Correlation within each block
factor <- 1
np <- length(adjustment.vec)
...
for (i in 1:np){
  adjustment<-adjustment.vec[i]
  cat("\n", "Random dependency setting with adjustment factor:", adjustment)
...
dependent_fdr2.mthd<-cbind(bh.fdr, lasla.dd.fdr)
dependent_etp2.mthd<-cbind(bh.etp, lasla.dd.etp)
...
matplot(1:length(adjustment.vec), dependent_fdr2.mthd, type="o", pch=1:4, lwd=2, 
        main="Network-setting2 FDR Comparison", 
        xlab="dependency", ylab="FDR", ylim=c(0.01, 0.10), xaxt="n")
axis(1, at=1:length(adjustment.vec), labels=adjustment.vec)

legend("top", c("BH","LASLA.DD"), pch=1:3, col=1:6, lwd=2)

matplot(1:length(adjustment.vec), dependent_etp2.mthd, type="o", pch=1:4, lwd=2, 
        main="Network-setting2 Power Comparison", 
        xlab="dependency", ylab="Power", xaxt="n")
axis(1, at=1:length(adjustment.vec), labels=adjustment.vec)
...
if (save){
  ...
  for (i in 1:length(method_names)) {
    tmp <- data.frame(Method = rep(method_names[i],length(adjustment.vec)),
                      FDR = dependent_fdr2.mthd[,i],
                      Power = dependent_etp2.mthd[,i],
                      Adjustment = adjustment.vec)
    results <- rbind(results, tmp)
  }
  save(results, file=sprintf("%s/dependent2.RData", data_dir))
}