...
m<-1000
mean<-3
gamma.vec<-seq(from=0, to=0.5, by=0.1)  # controls the asymmetry level
pis<-rep(0.1, m)
q<-0.05
np<-length(gamma.vec)
nrep<-1000
...
for (i in 1:nrep)
{
  ...
  { 
    ...
    set.seed(1000*i+j)   
    gamma<-gamma.vec[j]
    gamma<-runif(m, 0.5-gamma, 0.5+gamma)
...
if (save){
  ...
  save(results, file=sprintf("%s/heterogeneous_asymmetric.RData", data_dir))
}