...
nrep=1
q<-0.05
dis.vec <- seq(from=0, to=0.4, by=0.2)
np <- length(dis.vec)
...
adapt.fdr_list <- replicate(n_cluster, rep(0, np), simplify = FALSE)
adapt.etp_list <- replicate(n_cluster, rep(0, np), simplify = FALSE)
...
adapt.fdp_list<-replicate(n_cluster, matrix(rep(0, np*nrep), nrep, np), simplify = FALSE)
adapt.ntp_list<-replicate(n_cluster, matrix(rep(0, np*nrep), nrep, np), simplify = FALSE)
  ...
  for (j in 1:np){
    ...
    for (k in 1:m) {
      for (h in min((k+1),m):m) {
        d_lasla[k,h]=(theta[k]==theta[h])*abs(rnorm(1,dis,0.8))+(theta[k]!=theta[h])*abs(rnorm(1,1,0.8))
      }
    }
    for(k in 2:m) {
      for (h in 1:(k-1)) {
    ...
    for(k in seq_along(n_cluster.vec)) {
      ...
      # 
      adapt.res <-  adapt_glm(as.data.frame(s),pv,
                              pi_formulas = "1",
                              mu_formulas = "1",
                              verbose = list(print = FALSE, 
                                             fit = FALSE,
      ...
      adapt.fdp_list[[k]][i,j] <- sum((1-theta)[adapt.rej])/max(length(adapt.rej),1)
      adapt.ntp_list[[k]][i,j] <- sum(theta[adapt.rej])/sum(theta)
    }
  }
}
...
for (i in 1:np) {
  ...
  for(k in 1:length(n_cluster.vec)) {
    ...
    adapt.fdr_list[[k]][i]<-mean(adapt.fdp_list[[k]][,i])
    adapt.etp_list[[k]][i]<-mean(adapt.ntp_list[[k]][,i])
