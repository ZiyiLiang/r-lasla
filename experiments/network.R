...
mean.vec <- seq(from=2.5, to=2.6, by=0.1)
np <- length(mean.vec)
...
nw_etp1.mthd<-cbind(bh.etp, lasla.dd.etp, adaptMT.etp_list)
...
for (i in 1:nrep){
  ...
  for (j in 1:np){
    ...
    for (k in 1:m) {
      for (h in min((k+1),m):m) {
        d_lasla[k,h]=(theta[k]==theta[h])*abs(rnorm(1,dis,0.6))+(theta[k]!=theta[h])*abs(rnorm(1,1,0.6))
      }
    }
    for(k in 2:m) {
      for (h in 1:(k-1)) {
